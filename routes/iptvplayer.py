...
iptvplayer_bp = web.RouteTableDef()
...
@iptvplayer_bp.get('/iptvplayer')
async def player_page(request):
    """Renders the player HTML page."""
    try:
        with open('templates/iptvplayer.html', 'r', encoding='utf-8') as f:
            content = f.read()
...
@iptvplayer_bp.post('/player/parse')
...
@iptvplayer_bp.get('/api/parse-playlist')
...
@iptvplayer_bp.post('/player/epg/all')
